...
const LiveDetails = () => {
  const [showToolTip, setShowToolTip] = useState(false)
    ...
    const subMethod = async() => {
     ...
     await axios.post(process.env.NODE_ENV ==='production'?"https://spire-insights.herokuapp.com/api/user/bestSellers":'http://localhost:4000/api/user/bestSellers',{platform:currentPlatform, token:sessionStorage.getItem('token')}).then(res=> {
      ...
      await axios.post(process.env.NODE_ENV ==='production'?"https://spire-insights.herokuapp.com/api/user/newProductEntries":'http://localhost:4000/api/user/newProductEntries',
      {platform:currentPlatform, token:sessionStorage.getItem('token')}).then(res=> {
        setAnalysis(res.data)
        setLoading(false)
  }).catch(err=> {
      console.log(err)
     ...
     await axios.post(process.env.NODE_ENV ==='production'?"https://spire-insights.herokuapp.com/api/user/mostWished":'http://localhost:4000/api/user/mostWished',{platform:currentPlatform, token:sessionStorage.getItem('token')}).then(res=> {
        setAnalysis(res.data)
  ...
  const [initialSearched, setInitialSearched] = useState(false)
    ...
    return (
        ...
        <div class = 'relative mx-auto  z-[40]  w-fit'>
            <h1 class = 'sm:text-4xl text-3xl font-bold mt-[40px] mb-4 px-4 bottom-2 relative text-center'>Live Details</h1>
...
<TopProdResult loading={loading} operation = {operation} analysis = {analysis}/>
