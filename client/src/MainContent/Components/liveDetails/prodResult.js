...
import {GiPriceTag, GiClothes, GiLipstick, GiForkKnifeSpoon} from 'react-icons/gi'
...
<ClipLoader color={'#0A74FE'} loading={props.loading} size={110} />
      ...
      :
            ...
            props.analysis.map(elem=> {
               ...
               <div onClick={()=> {
                 ...
                 {returnIcon(elem.category)}
                  <h2 class = 'text-white font-bold uppercase sm:text-2xl text-xl text-center'>{elem.category.toLowerCase()==='improvement'?'Home':elem.category}</h2>
               </div>)
        ...
        <h1 class = 'sm:text-4xl text-3xl px-2 text-center bottom-6 mt-11 font-bold relative underline'>{
         ...
         'New Releases'
         :
         props.operation === 'wished'?'Most Wished Products':""
        }</h1>
                        <h3 class = 'text-xl text-center bottom-4 mb-9 font-semibold relative '>Live Insights - {category.charAt(0).toUpperCase() + category.substring(1)} Category</h3>
        
        
        <button onClick = {()=> {
         setCategory()
        }} class = 'bg-blue-700 text-white text-xl  shadow-sm hover:shadow-lg hover:bg-blue-600 px-4 py-2.5 rounded-md relative bottom-4 mt-2 mb-7 font-semibold mx-auto block'><AiOutlineRollback class = 'inline mr-1.5 relative bottom-[3px]'/>CATEGORIES</button>
           ...
           :''
                         ...
                         {props.analysis.map(function(categ) {
                           if(categ.category.toLowerCase() === category){
                              ...
                              return(
                                 Object.keys(categ).map(function(property,i) {
   ...
   <div key = {i} class={`w-full py-4  md:w-1/2 xl:w-1/3 px-4`}>
                               <div class="bg-white shadow-sm h-full rounded-lg overflow-hidden mb-1">
                                  ...
                                  <div class="p-8 sm:p-9 border-t-2 border-t-blue-700 border-dotted md:p-7 xl:p-9 xl:p-6 text-center">
                                     ...
                                     <GiPriceTag class = ' bottom-[2.75px] mr-[3px] text-blue-700 relative inline text-2xl'/>  AED <span class= 'text-2xl font-bold right-[0px] -ml-1 relative text-blue-700'>{categ[property]['price']?categ[property]['price'].split('AED')[1].replace('-',''):''}</span>
                                     </p>
                                     <p class="text-lg font-semibold text-body-color leading-relaxed mb-7">
                                     <MdOutlineStarRate class = ' bottom-[3px] text-yellow-600 left-[1px] relative inline text-2xl'/> Rated <span class= 'text-2xl font-bold  text-yellow-600'>{categ[property]['stars']?categ[property]['stars']:''}</span>/5
                                     </p>
                                     <a
                                       onClick={()=> {
       ...
       :
        ...
        <div class = 'h-[447px] -mb-4 bg-gradient-to-br mt-6 from-blue-100 to-indigo-200'>
                <img class = 'w-16 block top-[122px] mt-1 mx-auto relative' src = {logo}></img><br/>
                 ...
                 <h1 class = 'text-3xl px-3 font-bold text-center top-[122px] relative text-gray-500'>Live Market Details<br/> with <span class = 'text-blue-700'>Spire Insights</span></h1>
