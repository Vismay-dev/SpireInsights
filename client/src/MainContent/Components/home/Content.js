...
const ContentSetup = (props) => {
    ...
    axios
      .post(
        process.env.NODE_ENV === "production"
          ? "https://spire-insights.herokuapp.com/api/user/uploadPic"
      ...
      .then((res) => {
        ...
        if (userCon.user.pipeline.current !== "preparation") {
          ...
          axios
            .post(
              process.env.NODE_ENV === "production"
                ? "https://spire-insights.herokuapp.com/api/user/saveCurrentPipeline"
                : "http://localhost:4000/api/user/saveCurrentPipeline",
              {
                pipeline: { data: { ...currentPrep }, prepBeingEdited: true },
  ...
  useEffect(() => {
    axios
      .post(
        process.env.NODE_ENV === "production"
          ? "https://spire-insights.herokuapp.com/api/user/getUser"
      ...
      axios
        .post(
          process.env.NODE_ENV === "production"
            ...
            : "http://localhost:4000/api/user/saveCurrentPipeline",
          {
            pipeline: { data: { ...currentPrep }, keyWordsBeingEdited: true },
            token: sessionStorage.getItem("token"),
          }
        )
        .then((res) => {
      ...
      axios
        .post(
          process.env.NODE_ENV === "production"
            ...
            : "http://localhost:4000/api/user/saveCurrentPipeline",
          {
            pipeline: { ...userNow.pipeline, prepBeingEdited: true },
            token: sessionStorage.getItem("token"),
          }
        )
        .then((res) => {
  ...
  const subKeyWords = () => {
    ...
    axios
      .post(
        process.env.NODE_ENV === "production"
          ? "https://spire-insights.herokuapp.com/api/user/saveCurrentPipelineKeyWords"
          : "http://localhost:4000/api/user/saveCurrentPipelineKeyWords",
  ...
  const saveFunc = async (type) => {
    ...
    await axios
      .post(
        process.env.NODE_ENV === "production"
          ? "https://spire-insights.herokuapp.com/api/user/saveCurrentPipelinePrep"
          : "http://localhost:4000/api/user/saveCurrentPipelinePrep",
        { details, token: sessionStorage.getItem("token") }
      )
      .then((res) => {
        userCon.setUser({ ...userCon.user, pipeline: res.data });
  ...
  const contentList = [
    <div className="mt-12 relative mx-auto lg:mt-14 lg:mb-14 mb-12">
      <div class="lg:w-[75%] w-[85%] lg:left-[12.5%] left-[7.5%] bg-gray-800 relative shadow-xl rounded-lg rounded-bl-2xl rounded-br-2xl">
        <img
          className="w-full h-[280px] md:object-contain object-cover"
        ...
        <div className="bg-white sm:text-left text-center  sm:pl-10 sm:pr-2 px-2 block sm:mx-0 mx-auto  py-6 pt-8 rounded-bl-2xl rounded-br-2xl">
          <h1 className="md:text-4xl text-3xl pr-3 text-gray-900 sm:text-left text-center sm:mx-0 mx-auto block sm:px-0 px-3 font-semibold tracking-wider">
            E-Commerce Product Listing
          </h1>
          <p className="text-gray-700 text-base lg:text-lg sm:text-left text-center sm:mx-0 mx-auto block lg:leading-8 tracking-wide sm:mt-6 mt-7 w-11/12">
            Creating a new ecommerce product listing isn't as easy as you might
            think. With a few simple mistakes it's easy to create an entry which
            is either unappealing or doesn't contain enough information for your
            target audience to make a purchase.
          </p>
          <p className="text-gray-700 text-base lg:text-lg sm:text-left text-center sm:mx-0 mx-auto block lg:leading-8 tracking-wide sm:mt-4 mt-5 w-11/12">
            ...
            Those who{" "}
            <span class="text-indigo-600 font-semibold">
              conquer that trend will be market leaders.
            </span>
            " - John Phillips, Senior VP of Logistics @ Pepsico
          </p>
          <p className="text-gray-700 text-base lg:text-lg sm:text-left text-center sm:mx-0 mx-auto block lg:leading-8 tracking-wide sm:mt-4 mt-5 w-11/12">
            Our Step-by-Step guide will take you through the process of setting{" "}
            <span class="text-indigo-600 font-semibold">
              your E-Commerce Product Listings
            </span>{" "}
            up. From{" "}
            <span class="text-indigo-600 font-semibold">
              Set-Up Details & Search Engine Optimization
            </span>{" "}
            to{" "}
            <span class="text-indigo-600 font-semibold">
              Keyword Validation & Market Analysis
            </span>
            , we've got it all, and we're here to help you achieve success in
            the internet's toughest E-Commerce marketplaces.
          </p>
          ...
          <div class="w-full px-4">
            <div class="text-center relative left-2 mx-auto mb-12 lg:mb-14 lg:max-w-[800px] max-w-[700px]">
              ...
              <p class="text-base text-body-color relative top-3 mb-5 block">
                Use these instructions to systematically organize your
                E-Commerce Listings. An aspect ignored by most online business,
                comprehensive planning of listings is key to better consumer
                exposure & higher sales. Planning & storing these details early
                saves time later when setting up online stores.
              </p>
            </div>
            ...
            <a
              onClick={() => {
                if (
                  userCon.user &&
                  userCon.user.pipeline &&
                  userCon.user.pipeline.current !== "preparation"
                ) {
                  saveAllDetailsPrep();
                  props.changeSeg(3);
                }
            ...
            userCon.user.pipeline.current !== "preparation" ? (
              <div class="block mx-auto -mt-1 -top-2 mb-2 left-4 relative">
                {!userCon.user.pipeline.prepBeingEdited ? (
                  ...
                  <>
                    <ImCross class="text-red-600 text-center mx-auto relative mb-3 -top-[1px] text-4xl p-1 bg-gradient-to-br block from-blue-100 to-indigo-100 rounded-full" />{" "}
                    <h1 class="text-center uppercase mb-3 font-semibold underline">
                      Being Edited...
                    </h1>
                  </>
                )}
          ...
          <div class="w-full md:w-1/2 md:pb-10  px-4">
            ...
            >
              ...
              <p class="text-body-color">
                <span class="underline font-semibold -mb-9 block relative top-1.5">
                ...
                <br />
                - Product Name <br />- Rough Product Details
              </p>
              <hr class="mt-6" />
                ...
                <div class="w-full md:w-1/2 lg:w-1/2 px-2">
                  <div class="mb-12">
                    ...
                    <input
                      type="text"
                      name="platform"
                      defaultValue={storeDetails.platform}
                      onChange={storeDetailsChange}
                ...
                <div class="w-full md:w-1/2 lg:w-1/2 px-2">
                  <div class="mb-12">
                    ...
                    <input
                      type="text"
                      name="productName"
                      defaultValue={storeDetails.productName}
                      onChange={storeDetailsChange}
                ...
                <div class="w-full px-2 -mt-7">
                  <div class="mb-4">
                    ...
                    <input
                      type="text"
                      name="productDetails"
                      defaultValue={storeDetails.productDetails}
                      onChange={storeDetailsChange}
            ...
            >
              ...
              <p class="text-body-color">
                ...
                <p class="font-semibold inline">
                  ...
                  <span class="lg:hidden inline">and</span> default order size
                  (single or bulk),
                </p>{" "}
                <span class="xl:inline hidden">
                  decide and finalize your product's pricing.
                </span>{" "}
                <strong>
                  Use our optimization feature to{" "}
                  <span class="lg:inline hidden">evaluate the market &</span>{" "}
                  set a <span class="xl:inline hidden">competitive</span> price.
                </strong>
              </p>
              <hr class="mt-6" />
              <div class="w-full px-2 mt-6">
                <div class="mb-4">
                  ...
                  <input
                    ...
                    min={0}
                    name="productPricing"
                    defaultValue={pricing.productPricing}
              ...
              <div class="w-full px-2 mt-4">
                <div class="mb-4">
                  ...
                  <select
                    name="currency"
                    defaultValue={
                      pricing.currency === "" ? "AED" : pricing.currency
                    }
                    class="
            w-full
            border-[1.5px] border-form-stroke
...
"
          ...
          <div class="w-full md:w-1/2 pb-14  px-4">
            ...
            >
              ...
              <p class="text-body-color">
                <span class="underline  font-semibold -mb-9 block relative top-1.5">
                  Pre-plan these details:
                </span>
                <br />
                <br />
                - Product Dimensions & Weight
                <br />
                - Shipping Cost (Local & Abroad)
                <br />
                ...
                <div class="w-full md:w-1/2 lg:w-1/2 px-2">
                  <div class="mb-4">
                    ...
                    <input
                      ...
                      defaultValue={shipping.productDimension}
                ...
                <div class="w-full md:w-1/2 lg:w-1/2 px-2">
                  <div class="mb-4">
                    <label
                      ...
                      class="font-medium text-base text-black block mb-3"
                    >
                      Weight (<span class="lg:inline hidden">Estimated - </span>
                      Kg)
                    </label>
                    <input
                      onChange={shippingChange}
                      min={0}
                      name="productWeight"
                      defaultValue={shipping.productWeight}
                      type="number"
                      placeholder="Weight"
                      class="
         w-full
...
"
          ...
          <div class="w-full md:w-1/2 pb-14  px-4">
            ...
            >
              ...
              <span class="underline  font-semibold -mb-10 block relative top-0.5">
                Pre-plan these details:
              </span>
              <br />
              <br />
              ...
              <p class="text-body-color">
                - Use a plain background <br />- Use several angles{" "}
                <span class="xl:inline hidden">& suitable lighting </span>
                <br />- Show your product{" "}
                <span class="lg:inline hidden">in use</span> in{" "}
                <span class="lg:inline hidden">atleast one</span>
                <span class="lg:hidden inline">an</span> image
              </p>
              <hr class="mt-6" />
              <div class="flex flex-wrap -mx-4 mt-6">
          ...
          <div class="w-full px-4">
            <div class="text-center mx-auto mb-14 lg:mb-14 lg:max-w-[650px] max-w-[550px] ">
              ...
              <p class="text-base text-body-color relative top-3">
                Use the services listed below to select the highest-ranking
                keywords suitable to your product listing. Understand the
                Competition. Conquer the online marketplace. Win.
              </p>
            </div>
            ...
            <div class="grid bottom-2 md:w-[80%] sm:w-[95%] w-[100%] mx-auto relative mb-8  sm:grid-cols-2 grid-cols-1 gap-x-7">
              <div
                onClick={() => {
                  ...
                  if (isMobile) {
                    ...
                    window.open(
                      process.env.NODE_ENV === "production"
                        ? "https://spire-insights.herokuapp.com/optimiseListings"
                        : "http://localhost:3000/optimiseListings",
                      "_blank"
                    );
                  }
                }}
              ...
              >
                ...
                <h2 class="text-xl mt-[35px]  text-center font-semibold">
                  Analyze Top Products <br />
                  <hr class="w-2/3 mx-auto text-center block mt-1.5 my-[2px]" />{" "}
            ...
            <h1 class="text-2xl text-center mx-auto relative block font-semibold my-3">
              Enter 5 Keywords:
              <br />{" "}
              <p class="text-xl">
                (Select One of the Above Features to Find Keywords)
              </p>
            </h1>
            ...
            <div class="relative block mx-auto sm:top-[2px] top-[14px] mb-3.5">
              {keyWords.beingEdited ? (
                keyWords.isSaved ? (
                  ...
                  <>
                    <ImCross class="text-red-600 text-center mx-auto relative mb-2 text-4xl p-1 bg-gradient-to-br block from-blue-100 to-indigo-100 rounded-full" />{" "}
                    <h1 class="text-center uppercase mb-3 font-semibold underline">
                      Being Edited...
                    </h1>
                  </>
        ...
        >
          ...
          >
            ...
            >
              <svg
                ...
                height="10"
                viewBox="0 0 17 10"
                class={`fill-current icon relative bottom-0.5 ${
          ...
          <div class={`${isOpen ? "block" : "hidden"}  pl-[62px]`}>
            <p class="text-base text-body-color leading-relaxed py-3 pb-0">
              Whether you're using Amazon, this usually involves nothing more
              than logging in, navigating to your product collection, then
              clicking "New Listing" (or the equivalent).
            </p>
          </div>
        </div>
...
`}
          >
            <FiShoppingCart class="mr-2 text-xl top-[0.5px] font-bold relative" />{" "}
            Al Anees (Qatar)
          </a>
          <Tooltip
            show={showToolTip}
          ...
          <p class="font-medium top-2.5 block mx-auto text-center relative mb-1">
            {" "}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4 inline relative bottom-[1.3px]"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              stroke-width="2"
            ...
            This feature is currently <strong>unavailable</strong>....
            <br />{" "}
            <p class="relative mt-2 lg:w-[75%] md:w-[90%] w-[105%] md:right-0 right-[2.5%] text-center mx-auto block">
              Just by entering your Seller Central LWA token, you'll be able to{" "}
              <strong class="text-center">
                set-up, manage, refine & analyse all your product listings on
                Spire
              </strong>
              , soon.
            </p>
   ...
   <div class="w-full md:w-1/2 -left-[65px] lg:w-1/3 relative px-4 mx-auto">
      <div class="mb-2 relative top-[38px]">
         <button class = 'bg-blue-400 text-white px-5 py-[11px] bottom-[1px] relative rounded-md shadow-sm cursor-default'>Submit LWA Token</button>
            ...
            <img
              class="mx-auto block h-fit w-[130px]  sm:right-[12px] right-[20px] -mt-[25px] mb-7 relative top-[12px]"
              src={image}
            ></img>
          </div>
        </div>
          ...
          <a
            onClick={() => {
              if (
                userCon.user &&
                userCon.user.pipeline &&
                userCon.user.pipeline.current !== "seo"
              ) {
          ...
          userCon.user.pipeline.current !== "seo" ? (
            <div class="block mx-auto top-[30px] ml-1 mt-[7px] mb-7 relative">
              {!userCon.user.pipeline.keyWordsBeingEdited ? (
                ...
                <>
                  <ImCross class="text-red-600 text-center mx-auto relative mb-2 mt-1 text-4xl p-1 bg-gradient-to-br block from-blue-100 to-indigo-100 rounded-full" />{" "}
                  <h1 class="text-center uppercase mb-3 font-semibold underline">
                    Being Edited...
                  </h1>
                </>
