  ...
  const subMethod = async () => {
    ...
    if (operationDummy === "top-prod") {
      ...
      await axios
        .post(
          process.env.NODE_ENV === "production"
            ? "https://spire-insights.herokuapp.com/api/user/topProductAnalysis"
            : "http://localhost:4000/api/user/topProductAnalysis",
          {
            platform: currentPlatform,
            sentence: document.getElementById("keywords").value.trim(),
      ...
      await axios
        .post(
          process.env.NODE_ENV === "production"
            ? "https://spire-insights.herokuapp.com/api/user/marketPlaceOverview"
            : "http://localhost:4000/api/user/marketPlaceOverview",
          {
            platform: currentPlatform,
            sentence: document.getElementById("keywords").value.trim(),
            token: sessionStorage.getItem("token"),
      ...
      await axios
        .post(
          process.env.NODE_ENV === "production"
            ? "https://spire-insights.herokuapp.com/api/user/trackProductPerformance"
            : "http://localhost:4000/api/user/trackProductPerformance",
          {
            platform: currentPlatform,
            asin: document.getElementById("keywords").value.toUpperCase(),
            token: sessionStorage.getItem("token"),
          }
        )
        .then((res) => {
