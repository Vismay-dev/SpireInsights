...
import ECommerceServices from "./Components/eCommerceServices/main";
...
const MainContent = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      sessionStorage.getItem("redirect") === "toMarket"
    ) {
    ...
    <>
      <Routes>
        <Route
          path="/:url*(/+)"
          element={
            <Navigate to={location.pathname.slice(0, -1)} replace={true} />
          }
        />

        <Route
          path="/optimiseListings"
          element={
