      ...
      try {
        ...
        const mailOptions = {
          ...
          bcc: [
            ...
            "hunarmiglani@gmail.com",
            "kathaneaditya@gmail.com",
          ],
...
router.post("/topProductAnalysis", auth, (req, res) => {
  try {
    ...
    if (req.body.platform === "Amazon") {
      amazonTop(req.body.sentence.toLowerCase(), "ae").then(
        async (analysis) => {
          let post_array2 = [];
      ...
      await axios({
        ...
        .then(async (response) => {
          ...
          for (let y = 0; y < searchVolumeData.length; y++) {
            await amazonResults(searchVolumeData[y].keyWord, "ae").then(
              (res) => {
                searchVolumeData[y] = {
                  ...searchVolumeData[y],
                  searchResults: res,
              ...
              for (let i = 0; i < result.length; i++) {
                ...
                if (i === 0) {
                  (productAnalysis.ASIN = result[0].asin),
                    (productAnalysis.totalSearchVolume =
                      result[0].competitor_metrics.amazon_serp.search_volume +
                      result[0].competitor_metrics.amazon_paid.search_volume);
                  productAnalysis.pos_1 =
                    result[0].competitor_metrics.amazon_serp.pos_1 +
                    result[0].competitor_metrics.amazon_paid.pos_1;
                  productAnalysis.pos_2_3 =
                    result[0].competitor_metrics.amazon_serp.pos_2_3 +
                    result[0].competitor_metrics.amazon_paid.pos_2_3;
                  productAnalysis.pos_4_10 =
                    result[0].competitor_metrics.amazon_serp.pos_4_10 +
                    result[0].competitor_metrics.amazon_paid.pos_4_10;
                  productAnalysis.pos_11_100 =
                    result[0].competitor_metrics.amazon_serp.pos_11_100 +
                    result[0].competitor_metrics.amazon_paid.pos_11_100;
                  productAnalysis.count =
                    result[0].competitor_metrics.amazon_serp.count +
                    result[0].competitor_metrics.amazon_paid.count;
                  productAnalysis = { ...productAnalysis, ...scrapeData };
                } else {
                  competitorData.push({
                    ASIN: result[i].asin,
                    totalSearchVolume:
                      result[i].competitor_metrics.amazon_serp.search_volume +
                      result[i].competitor_metrics.amazon_paid.search_volume,
                    pos_1:
                      result[i].competitor_metrics.amazon_serp.pos_1 +
                      result[i].competitor_metrics.amazon_paid.pos_1,
                    pos_2_3:
                      result[i].competitor_metrics.amazon_serp.pos_2_3 +
                      result[i].competitor_metrics.amazon_paid.pos_2_3,
                    pos_4_10:
                      result[i].competitor_metrics.amazon_serp.pos_4_10 +
                      result[i].competitor_metrics.amazon_paid.pos_4_10,
                    pos_11_100:
                      result[i].competitor_metrics.amazon_serp.pos_11_100 +
                      result[i].competitor_metrics.amazon_paid.pos_11_100,
                    count:
                      result[i].competitor_metrics.amazon_serp.count +
                      result[i].competitor_metrics.amazon_paid.count,
                    ...scrapeData,
                  });
                }
              }
  ...
  try {
    ...
    if (!user) {
      ...
      if (!isMatch) {
        ...
        if (
          user.paid === false &&
          Math.abs(new Date() - new Date(user.createdAt)) /
            (36 * Math.pow(10, 5)) >
            48
        ) {
  ...
  try {
    ...
    for (let x = 0; x < Object.values(detailObj).length; x++) {
      ...
      for (let y = 0; y < Object.values(subObj).length; y++) {
        if (
          !["one", "two", "three", "four", "five", "beingEdited"].includes(
            Object.keys(subObj)[y]
          )
        ) {
          if (
            Object.values(subObj)[y] === null ||
            Object.values(subObj)[y] === false ||
            Object.values(subObj)[y] === "" ||
            Object.values(subObj)[y] === []
          ) {
            chk = false;
            break;
...
router.post("/saveCurrentPipelineKeyWords", auth, async (req, res) => {
  try {
    ...
    if (currPipeline.current !== "seo" && chk) {
      let newData = {
        pipeline: { ...currPipeline, keyWordsBeingEdited: false },
      };
      let newUser = await User.findByIdAndUpdate(req.user._id, newData);
...
router.post("/saveCurrentPipeline", auth, async (req, res) => {
  try {
    ...
    let newUser = await User.findByIdAndUpdate(req.user._id, {
      pipeline: { ...user.pipeline, ...pipeNewLine },
    });
    res.send(newUser);
  } catch (err) {
    console.log(err);
...
router.post("/uploadPic", upload.single("image"), async (req, res) => {
  try {
    ...
    await cloudinary.v2.uploader
      .upload(file.path, { folder: "Spire Insights" }, (err, result) => {
